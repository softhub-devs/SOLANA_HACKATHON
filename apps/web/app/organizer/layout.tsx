...
import {
  clearOrganizerOrgId,
  persistOrganizerOrgId,
  readOrganizerOrgId,
} from "@/lib/organizerStorage";

function getSidebarItems(orgId: string | null) {
  const dashboardHref = orgId ? `/organizer/${orgId}` : "/organizer";
  const tournamentsHref = orgId ? `/organizer/${orgId}/tournament` : "/organizer";
...
export default function DashboardLayout({
  ...
  useEffect(() => {
    if (routeOrgId) {
      persistOrganizerOrgId(routeOrgId);
      setStoredOrgId(routeOrgId);
      return;
    }

    setStoredOrgId(readOrganizerOrgId());
  }, [routeOrgId]);

  useEffect(() => {
    if (routeOrgId || storedOrgId) {
      return;
    }

    let mounted = true;

    async function syncOrganizerOrgId() {
      try {
        const payload = await apiFetch<SessionResponse>("/auth/session");
        if (!payload.user.orgId || !mounted) {
          return;
        }

        persistOrganizerOrgId(payload.user.orgId);
        setStoredOrgId(payload.user.orgId);
      } catch {
        if (mounted) {
          setStoredOrgId(null);
        }
    ...
    void syncOrganizerOrgId();

    return () => {
      mounted = false;
    };
  }, [routeOrgId, storedOrgId]);

  const orgId = routeOrgId ?? storedOrgId;
  const sidebarItems = getSidebarItems(orgId ?? null);
  ...
  async function handleLogout() {
    ...
    try {
      ...
      // Best effort logout: continue clearing local organizer state.
    } finally {
      clearOrganizerOrgId();
      setStoredOrgId(null);
  ...
  return (
    <div className="flex min-h-screen bg-[#020617] text-white">
      {/* SIDEBAR */}
      <aside className="hidden w-[270px] border-r border-white/10 bg-slate-950/70 lg:flex lg:flex-col">
          ...
          <div className="flex h-[76px] items-center justify-between px-6">
            ...
            <div>
              <p className="text-sm text-slate-400">
                GameChain / Organizer{orgId ? ` / ${orgId}` : ""}
              </p>
